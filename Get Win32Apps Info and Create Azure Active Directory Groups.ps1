...
Connect-AzureAD -TenantId "xenappblog.com"
Connect-MSIntuneGraph -TenantID "xenappblog.com"

$apps = Get-IntuneApplication | Where-Object { $_.'@odata.type' -eq "#microsoft.graph.win32LobApp" -and $_.filename -ne "PatchMyPC-ScriptRunner.intunewin" -and $_.DisplayName -notlike "Update*" }
$apps = $apps | Where-Object { $_.DisplayName -notlike "Evergreen*" }
...
ForEach($App in $Apps) {
    ...
    if ($Publisher -eq "Microsoft Corporation") { $Publisher = "Microsoft" }
    if ($Publisher -eq "Citrix Systems, Inc.") { $Publisher = "Citrix" }
    if ($Publisher -eq "Adobe Systems, Inc.") { $Publisher = "Adobe" }
    
    $Name = $Name -replace " $Version",""

    $AADGroup = "AAD - SW - $Publisher" + " " + "-" + " " + "$Name"
    
    Write-Verbose "Checking if Azure AD Group exists" -Verbose
    $Exists = Get-AzureADGroup -SearchString $AADGroup

    if ($Exists -eq $null -and $Name -notlike "Visual C*") {
        Write-Verbose "Creating Azure AD Group : $AADGroup" -Verbose
        New-AzureADGroup -DisplayName "$AADGroup" -MailEnabled $false -SecurityEnabled $true -MailNickName "NotSet" | Out-Null
        $AADGID = Get-AzureADGroup -SearchString $AADGroup
        $AADGID = $AADGID.ObjectId
        Write-Verbose "Adding Group ID $AADGID to $Name" -Verbose
        Add-IntuneWin32AppAssignmentGroup -ID $AppID -GroupID $AADGID -Intent required -Include | Out-Null
    }
    else
    {
        Write-Warning "Azure AD Group $AADGroup Already Exists"
    }
