...
$apps = Get-IntuneApplication | Where-Object { $_.'@odata.type' -eq "#microsoft.graph.win32LobApp" -and $_.filename -ne "PatchMyPC-ScriptRunner.intunewin" -and $_.DisplayName -like "Update*" }
...
ForEach($App in $Apps) {
    ...
    Write-Verbose "Removing All Existing Users from $AADGroup" -Verbose    
    ForEach ($CurrentMember in $(Get-AzureADGroupMember -ObjectID $AADGID -All $True)) { 
        Remove-AzureADGroupMember -ObjectId $AADGID -MemberId $($CurrentMember.ObjectId)
    }

    ForEach($User in $Users) {
       Write-Verbose "Adding $User to $AADGroup" -Verbose
       $ADUserID = Get-AzureADUser -ObjectId $User | Select-Object ObjectID -ExpandProperty ObjectID
